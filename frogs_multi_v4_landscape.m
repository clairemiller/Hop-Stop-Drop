...
clear; clc; close all;
%habitat = h5read("landscape_matrix.h5",'/category_id');
habitat = h5read("quail_habitat_matrix.h5",'/landscape');
habitat(habitat == 0) = 4;
habitat = habitat';

%% ---- PARAMETERS ----------------------------------------
[Lx,Ly]  = size(habitat);     % Grid size (Lx x Ly)
...
fi = randi(Lx, 1, n_frogs);   % row of each frog
fi(:) = round(Lx/2);
%fi(:) = 500;
fj = randi(Ly, 1, n_frogs);   % column of each frog
fj(:) = round(Ly/2);
%fj(:) = 1700;
...
for t = 1:T_steps
    ...
    for k = 1:n_frogs
       ...
       % 2. Probability of movement depending on the habitat
        ...
        [Pr_r, Pr_u, Pr_l, Pr_d] = grad_sector(fi(k), fj(k), sgn, Lx,Ly, ...
...
function [Pr_r, Pr_u, Pr_l, Pr_d] = grad_sector(fi, fj, sgn, Lx,Ly, ...
