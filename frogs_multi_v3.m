...
frog_temp = get_habitat_temp(T_hab_table, frog_hab, T0);
...
for t = 1:T_steps
    ...
    for k = 1:n_frogs
       ...
       % 2. Probability of movement depending on the habitat
        ...
        % Temperature gradient in each direction (0 if out of bounds)
        dT_r = grad(fi(k),   fj(k)+1, sgn, N, T_hab_table, hab_actual, habitat, time_mins);
        dT_u = grad(fi(k)-1, fj(k), sgn, N, T_hab_table, hab_actual, habitat, time_mins);
        dT_l = grad(fi(k),   fj(k)-1, sgn, N, T_hab_table, hab_actual, habitat, time_mins);
        dT_d = grad(fi(k)+1, fj(k), sgn, N, T_hab_table, hab_actual, habitat, time_mins);
 
        c_norm = dT_r + dT_u + dT_l + dT_d;
 
        if c_norm > 0
            Pr_r = dT_r / c_norm;
            Pr_u = dT_u / c_norm;
            Pr_l = dT_l / c_norm;
            % Pr_d = dT_d / c_norm  (implicitly = 1 - others)
        else
            % No temperature gradient visible -> skip MOVE 2
            Pr_r = 0;  Pr_u = 0;  Pr_l = 0;
            phi = 0;
        end
        ...
        if c2 < phi * Pr_r
            dc2 =  1;                              % right
        elseif c2 < phi * (Pr_r + Pr_u)
            dr2 = -1;                              % up
        elseif c2 < phi * (Pr_r + Pr_u + Pr_l)
            dc2 = -1;                              % left
        elseif c2 < phi
            dr2 =  1;                              % down
        end
